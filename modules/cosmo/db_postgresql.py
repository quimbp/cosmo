...
def get_buoys_from_exp(db,name):
    ...
    ps = db.prepare(query)
    items = [ps()[i][0] for i in range(len(ps()))]
    return items
...
def get_exp(db):
    ...
    ps = db.prepare(query)
    items = [list(ps()[i][:]) for i in range(len(ps()))]
    return items
...
# =================
  ...
  ttk.Button(F0,text='Download a copy',command=_get).grid(row=1,column=4,columnspan=3,padx=0)
